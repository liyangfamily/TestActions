...
QRect MCCusSwitchButton::indicatorRect() {
...
QRect MCCusSwitchButton::textRect() {
	const auto w = style.indicatorMargin.left() + style.height + style.indicatorMargin.right();
	return ltr(this) ? rect().marginsRemoved(QMargins(w, 0, 0, 0)) : rect().marginsRemoved(QMargins(0, 0, w, 0));
}

MCCusSwitchButton::MCCusSwitchButton(QWidget* parent) : SelectionControl(parent) {
	init();
}

MCCusSwitchButton::MCCusSwitchButton(const QString& text, QWidget* parent) : MCCusSwitchButton(parent) {
	setText(text);
}

MCCusSwitchButton::MCCusSwitchButton(const QString& text, const QBrush& brush, QWidget* parent) : MCCusSwitchButton(text, parent) {
	style.thumbOnBrush = brush.color();
	style.trackOnBrush = brush.color();
}

MCCusSwitchButton::~MCCusSwitchButton() {

}

QSize MCCusSwitchButton::sizeHint() const {
	auto h = style.height;
...
void MCCusSwitchButton::paintEvent(QPaintEvent*) {
	...
	if (isEnabled()) {
        ...
        p.drawEllipse(thumbRect.center(), THUMB_RADIUS - SHADOW_ELEVATION - 4.0, THUMB_RADIUS - SHADOW_ELEVATION - 4.0);
		p.setRenderHint(QPainter::Antialiasing, false);
...
void MCCusSwitchButton::setSwitchStatus(bool status)
{
	if (status != isChecked()) {
		setChecked(status);
	}
