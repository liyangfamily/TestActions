...
int LBL_TaskEngine_IntegratedCtrlTask::filter_identifyProtocolHeader(const LBLEnginePackage& block, int offset)
{
    const int blocklen = block.data().length();
    const char * dataptr = block.data().constData();
    //先检测是否满足协议头
    ...
    else//协议头刚刚完成
    {
        ...
        if(_currentProtocolHeader.protocolVersion==0x03){
            return filter_identifyStandardHeader3_0(block,offset);
        }
        else{
            qDebug() << "Filter Message: Protocol Version Not Match , Version is " << _currentProtocolHeader.protocolVersion <<".";
            m_currentMessageSize = 0;
            m_currentBlock = QByteArray();
            offset = blocklen;
        }
...
int LBL_TaskEngine_IntegratedCtrlTask::filter_identifyStandardHeader3_0(const LBLEnginePackage& block, int offset)
{
    const int blocklen = block.data().length();
    const char * dataptr = block.data().constData();
    //再检测是否满足标准头
