...
#define LBL_TAG_SENDERCARDPCON600 "PCON600"
namespace LBL
{
    namespace SCItem
    {
        ...
        int SenderCardItem_PCON600::maxPortCount() const
        {
            return 18;
        }

        bool SenderCardItem_PCON600::hasMS9570() const
        {
            return false;
        }

        bool SenderCardItem_PCON600::hasNT68400() const
        {
            return true;
        }

        bool SenderCardItem_PCON600::hasAndroid() const
        {
            return true;
        }
        ...
        quint16 SenderCardItem_PCON600::writeSCNT68400Resolution(quint8 value, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_SenderCardCtrl->writeNT68400Resolution(value, sync, msec);
        }

        QByteArray SenderCardItem_PCON600::readSCNT68400Resolution(bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_SenderCardCtrl->readNT68400Resolution(sync, msec);
        }

        quint8 SenderCardItem_PCON600::getSCNT68400Resolution()
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_SenderCardCtrl->getNT68400Resolution();
        }

        quint16 SenderCardItem_PCON600::writeSC10BitSource(quint8 value, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            if(!value){
        ...
        QByteArray SenderCardItem_PCON600::readSC10BitSource(bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_SenderCardCtrl->read10BitSource(sync, msec);
        }

        quint8 SenderCardItem_PCON600::getSC10BitSource()
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_SenderCardCtrl->get10BitSource();
        }

        quint16 SenderCardItem_PCON600::writeSCLowDelay(quint8 value, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_SenderCardCtrl->writeLowDelay(value, sync, msec);
        }

        QByteArray SenderCardItem_PCON600::readSCLowDelay(bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_SenderCardCtrl->readLowDelay(sync, msec);
        }

        quint8 SenderCardItem_PCON600::getSCLowDelay()
        {
            Q_D(SenderCardItem_PCON600);
        ...
        quint16 SenderCardItem_PCON600::writeConnection(LBLConnection * value, bool sync, int msec)
        {
            ...
            quint16 ret = writeSCFPGAEthLoad(value->portList(), sync, msec);
        ...
        quint16 SenderCardItem_PCON600::clearDeadPoint(quint8 port, quint8 module, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_ReceiveCardCtrl->clearDeadPoint(port, module, sync, msec);
        }

        quint16 SenderCardItem_PCON600::writeXMICParam(quint8 port, quint8 module, QByteArray value, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_ReceiveCardCtrl->writeXMICParam(port, module, value, sync, msec);
        }

        quint16 SenderCardItem_PCON600::writeLinearTableFile(quint8 port, quint8 module, QByteArray value, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_ReceiveCardCtrl->writeLinearTableFile(port, module, value, sync, msec);
        }
        QByteArray SenderCardItem_PCON600::readXMICParam(quint8 port, quint8 module, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_ReceiveCardCtrl->readXMICParam(port, module, sync, msec);
        }
        ...
        quint16 SenderCardItem_PCON600::writeLowGrayCalibrationDataErase(quint8 port, quint8 module, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_ReceiveCardCtrl->writeLowGrayCalibrationDataErase(port, module, sync, msec);
        }

        quint16 SenderCardItem_PCON600::writeLowGrayCalibrationDataSave(quint8 port, quint8 module, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_ReceiveCardCtrl->writeLowGrayCalibrationDataSave(port, module, sync, msec);
        }

        quint16 SenderCardItem_PCON600::writeLowGrayCalibrationDataReload(quint8 port, quint8 module, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
        ...
        quint16 SenderCardItem_PCON600::readAllHDMIAdvanceData_PIP(quint8 param, quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->readAllDPAdvanceData_PIP(param,channel,sync,msec);
        }

        quint16 SenderCardItem_PCON600::writeHDMIOutputSource(quint8 hdmi1, quint8 hdmi2, quint8 dp, quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->writeSetting_DP_TX(hdmi1, hdmi2, dp, channel, sync, msec);
        }

        quint16 SenderCardItem_PCON600::readHDMIOutputSource(quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->readSetting_DP_TX(channel, sync, msec);
        }

        QList<quint8> SenderCardItem_PCON600::getHDMIOutputSource()
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->getSetting_DP_TX();
        }

        quint16 SenderCardItem_PCON600::writeDP_TX_Pcon600(quint8 hdmi1, quint8 hdmi2, quint8 dp, quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->writeSetting_DP_TX_Pcon600(hdmi1, hdmi2, dp, channel, sync, msec);
        }

        quint16 SenderCardItem_PCON600::readDP_TX_Pcon600(quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->readSetting_DP_TX_Pcon600(channel, sync, msec);
        }

        bool  SenderCardItem_PCON600::getDP_TX_Pcon600(quint8 &hdmi1, quint8 &hdmi2, quint8 &dp)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->getSetting_DP_TX_Pcon600(hdmi1, hdmi2, dp);
        }

        quint16 SenderCardItem_PCON600::writeDP_TX_2ndPlus(quint8 param, quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->writeSetting_DP_TX_2ndPlus(param, channel, sync, msec);
        }

        quint16 SenderCardItem_PCON600::readDP_TX_2ndPlus(quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->readSetting_DP_TX_2ndPlus(channel, sync, msec);
        }

        quint8 SenderCardItem_PCON600::getDP_TX_2ndPlus()
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->getSetting_DP_TX_2ndPlus();
        }
        ...
        quint16 SenderCardItem_PCON600::writeHDIMIAudioVolume(quint8 param, quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->writeSetting_Volume(param, channel, sync, msec);
        }

        quint16 SenderCardItem_PCON600::readHDIMIAudioVolume(quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->readSetting_Volume(channel, sync, msec);
        }

        quint8 SenderCardItem_PCON600::getHDIMIAudioVolume()
        {
            Q_D(SenderCardItem_PCON600);
        ...
        quint16 SenderCardItem_PCON600::writeHDIMISharpness(quint8 param, quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->writePQ_Sharpness(param, channel, sync, msec);
        }

        quint16 SenderCardItem_PCON600::readHDIMISharpness(quint8 channel, bool sync, int msec)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->readPQ_Sharpness(channel, sync, msec);
        }

        quint8 SenderCardItem_PCON600::getHDIMISharpness()
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_NT68400Ctrl->getPQ_Sharpness();
        }

        quint16 SenderCardItem_PCON600::writeHDIMIDisplayArea(QSize value, quint8 channel, bool sync, int msec)
        {
            Q_UNUSED(channel);
            Q_D(SenderCardItem_PCON600);
        ...
        quint16 SenderCardItem_PCON600::readHDIMIDisplayArea(quint8 channel, bool sync, int msec)
        {
            Q_UNUSED(channel);
            Q_D(SenderCardItem_PCON600);
        ...
        QSize SenderCardItem_PCON600::getHDIMIDisplayArea()
        {
            Q_D(SenderCardItem_PCON600);
        ...
        quint16 SenderCardItem_PCON600::upgradeFile(bool sync, int msec, quint16 fileType, QString fileName, quint8 portIndex, quint16 moduleIndex)
        {
            Q_D(SenderCardItem_PCON600);
            return d->m_FileTransfer->common_UpgradeFile(sync, msec, fileType, fileName,portIndex,moduleIndex);
        }

        quint16 SenderCardItem_PCON600::upgradeFile(bool sync, int msec, quint16 fileType, QByteArray data, quint8 portIndex, quint16 moduleIndex)
        {
            Q_D(SenderCardItem_PCON600);
        ...
        QByteArray SenderCardItem_PCON600::GetEDIDData()
        {
            Q_D(SenderCardItem_PCON600);
            if (d->m_NT68400Ctrl) {
                return d->m_NT68400Ctrl->GetEDIDData();
            }
            else {
                return nullptr;
            }
          ...
          quint16 SenderCardItem_PCON600::writeFacTest_UpdateEDID(quint8 dataGroupIndex, quint8* pDataBuff, quint8 dataLen, bool sync, int msec)
          {
              Q_D(SenderCardItem_PCON600);
              if (d->m_NT68400Ctrl) {
