...
#define LBL_TAG_SENDERCARD2NDGZ "PCON500_GZ"
namespace LBL
{
    namespace SCItem
    {
        ...
        int SenderCardItem_2nd_GZ::maxPortCount() const
        {
            return 8;
        }

        bool SenderCardItem_2nd_GZ::hasMS9570() const
        {
            if(d_func()->m_detectInfo.communType == ECommunicatType::ECT_COM){
                return false;
            }
            else{
                return true;
        ...
        bool SenderCardItem_2nd_GZ::hasNT68400() const
        {
            return false;
        }

        bool SenderCardItem_2nd_GZ::hasAndroid() const
        {
            if(d_func()->m_detectInfo.communType == ECommunicatType::ECT_COM){
                return false;
            }
        ...
        quint16 SenderCardItem_2nd_GZ::writeSCNT68400Resolution(quint8 value, bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_SenderCardCtrl->writeNT68400Resolution(value, sync, msec);
        }

        QByteArray SenderCardItem_2nd_GZ::readSCNT68400Resolution(bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_SenderCardCtrl->readNT68400Resolution(sync, msec);
        }

        quint8 SenderCardItem_2nd_GZ::getSCNT68400Resolution()
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_SenderCardCtrl->getNT68400Resolution();
        }

        quint16 SenderCardItem_2nd_GZ::writeSC10BitSource(quint8 value, bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            if(!value){
        ...
        QByteArray SenderCardItem_2nd_GZ::readSC10BitSource(bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_SenderCardCtrl->read10BitSource(sync, msec);
        }

        quint8 SenderCardItem_2nd_GZ::getSC10BitSource()
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_SenderCardCtrl->get10BitSource();
        }

        quint16 SenderCardItem_2nd_GZ::writeSCLowDelay(quint8 value, bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_SenderCardCtrl->writeLowDelay(value, sync, msec);
        }

        QByteArray SenderCardItem_2nd_GZ::readSCLowDelay(bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_SenderCardCtrl->readLowDelay(sync, msec);
        }

        quint8 SenderCardItem_2nd_GZ::getSCLowDelay()
        {
            Q_D(SenderCardItem_2nd_GZ);
        ...
        quint16 SenderCardItem_2nd_GZ::writeLowGrayCalibrationDataErase(quint8 port, quint8 module, bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_ReceiveCardCtrl->writeLowGrayCalibrationDataErase(port, module, sync, msec);
        }

        quint16 SenderCardItem_2nd_GZ::writeLowGrayCalibrationDataSave(quint8 port, quint8 module, bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_ReceiveCardCtrl->writeLowGrayCalibrationDataSave(port, module, sync, msec);
        }

        quint16 SenderCardItem_2nd_GZ::writeLowGrayCalibrationDataReload(quint8 port, quint8 module, bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
		...
		{
			Q_D(SenderCardItem_2nd_GZ);
			quint16 ret = d->m_HDMIChip->readRGain(sync, msec);
			if (LAPI::EResult(ret) != LAPI::EResult::ER_Success){
				return ret;
			}
			ret = d->m_HDMIChip->readGGain(sync, msec);
			if (LAPI::EResult(ret) != LAPI::EResult::ER_Success){
				return ret;
			}
			ret = d->m_HDMIChip->readBGain(sync, msec);
			if (LAPI::EResult(ret) != LAPI::EResult::ER_Success){
				return ret;
			}
			return ret;
        ...
        quint16 SenderCardItem_2nd_GZ::readHDMIInputSignalInfo(bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_MS9570Ctrl->readInputResolutionInfo(sync,msec);
        }

        QString SenderCardItem_2nd_GZ::getHDMIInputSignalInfo()
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_MS9570Ctrl->getInputResolutionInfo();
        }
        ...
        quint16 SenderCardItem_2nd_GZ::writeHDIMIDisplayArea(QSize value, quint8 channel, bool sync, int msec)
        {
            Q_UNUSED(channel);
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_MS9570Ctrl->writeOutputResolutionInfo(value.width(),value.height(),sync,msec);
        }

        quint16 SenderCardItem_2nd_GZ::readHDIMIDisplayArea(quint8 channel, bool sync, int msec)
        {
            Q_UNUSED(channel);
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_MS9570Ctrl->readOutputResolutionInfo(sync,msec);
        }

        QSize SenderCardItem_2nd_GZ::getHDIMIDisplayArea()
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_MS9570Ctrl->getOutputResolutionInfo();
        }

        quint16 SenderCardItem_2nd_GZ::writeHDIMIAudioVolume(quint8 param, quint8 channel, bool sync, int msec)
        {
            Q_UNUSED(channel);
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_HDMIChip->writeVolume(param, sync, msec);
        }

        quint16 SenderCardItem_2nd_GZ::readHDIMIAudioVolume(quint8 channel, bool sync, int msec)
        {
            Q_UNUSED(channel);
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_HDMIChip->readVolume(sync, msec);
        }

        quint8 SenderCardItem_2nd_GZ::getHDIMIAudioVolume()
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_HDMIChip->getVolume();
        }

        quint16 SenderCardItem_2nd_GZ::resetHDMISetting(bool sync, int msec)
        {
            Q_D(SenderCardItem_2nd_GZ);
            return d->m_MS9570Ctrl->writeReset(sync,msec);
        }
        ...
        quint16 SenderCardItem_2nd_GZ::upgradeFile(bool sync, int msec, quint16 fileType, QString fileName, quint8 portIndex, quint16 moduleIndex)
        {
            ...
            if (d->m_AndroidFileTransfer&&fileType == LBLFileTransferPackage::EFileType::EFT_SenderMonitor) {
                ...
                return d->m_AndroidFileTransfer->common_UpgradeFile(sync, msec, fileType, fileName,portIndex,moduleIndex);
            }
            d->m_FileTransfer = d->m_MCUFileTransfer;
            return d->m_MCUFileTransfer->forMCU_UpgradeFile(sync, msec, fileType, fileName);
        }

        quint16 SenderCardItem_2nd_GZ::upgradeFile(bool sync, int msec, quint16 fileType, QByteArray data, quint8 portIndex, quint16 moduleIndex)
        {
            Q_D(SenderCardItem_2nd_GZ);
            if (d->m_AndroidFileTransfer &&
                ...
                d->m_FileTransfer = d->m_AndroidFileTransfer;
                return d->m_AndroidFileTransfer->common_UpgradeFile(sync, msec, fileType, data,portIndex,moduleIndex);
            }
            d->m_FileTransfer = d->m_MCUFileTransfer;
            return d->m_MCUFileTransfer->forMCU_UpgradeFile(sync, msec, fileType, data,portIndex,moduleIndex);
        }
        ...
        QByteArray SenderCardItem_2nd_GZ::GetEDIDData()
        {
            Q_D(SenderCardItem_2nd_GZ);
            if (d->m_MS9570Ctrl) {
                return d->m_MS9570Ctrl->GetEDIDData();
            }
            else {
                return nullptr;
            }
        ...
        quint16 SenderCardItem_2nd_GZ::writeHDMIEDID(int dataGroupIndex, quint8* pDataBuff,int dataLen)
        {
            Q_D(SenderCardItem_2nd_GZ);
